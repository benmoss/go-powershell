...
func (c *context) basicTest(t *testing.T) {
	sout, serr, err := c.shell.Execute(`echo "hi from stdout"; [Console]::Error.WriteLine("hi from stderr")`)
	if err != nil {
...
func (c *context) boundaryTest(t *testing.T) {
	...
	for i := 0; i < 1000; i++ {
		...
		select {
		case <-timeout:
			fmt.Printf("Out so far: %q\n", string(c.shell.(*shell).debugout))
			fmt.Printf("Err so far: %q\n", string(c.shell.(*shell).debugerr))
